...
var min = Math.floor(Math.random() * 2);
  ...
  var money = data.money
      if(money < 10000000000000000000000000000000000000000000000) api.sendMessage("Bạn cần 10000000000000000000000000000000000000000000000 đô để xem ảnh ?",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 10000000000000000000000000000000000000000000000})
   var callback = () => api.sendMessage({body:`Ảnh NUDE !!!!\nSố Ảnh: ${link.length}\n-10000000000000000000000000000000000000000000000 đô !`,attachment: fs.createReadStream(__dirname + "/cache/1.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.jpg"), event.messageID); 
